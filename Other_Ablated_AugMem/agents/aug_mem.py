...
class AugMem(nn.Module):
    ...
    def __init__(self, agent_config):
        ...
        self.memsize = 8
        self.compressedWidth = 13
        
        ## mobilenetv2
#         self.compressedChannel = 64
#         self.memsize = 8
#         self.compressedWidth = 14
...
#    # replay old reading attention 
    def criterion_replay(self, target):
        ...
        if replay_batchsize > 0:
            ...
            ReplayAttRead = ReplayStorage[:,:int(self.compressedChannel/self.memsize)*self.compressedWidth*self.compressedWidth*self.MemtopK]
    ...
    def updateStorage_epoch(self, train_loader):
        ...
        for i, (inputs, target) in enumerate(train_loader):
            ...
            att_read = att_read.detach().cpu().view(-1,self.compressedWidth,self.compressedWidth,int(self.compressedChannel/self.memsize),self.MemNumSlots)
            #print(att_read.shape)
        ...
        for cls in range(self.config['n_class']):
            if cls in self.StorageAttRead.keys():
                ...
                sz_w = self.StorageAttRead[cls][:,:self.compressedWidth*self.compressedWidth*int(self.compressedChannel/self.memsize)*self.MemtopK].size(0)
                sz_h = self.StorageAttRead[cls][:,:self.compressedWidth*self.compressedWidth*int(self.compressedChannel/self.memsize)*self.MemtopK].size(1)
                indices = torch.reshape(self.StorageAttRead[cls][:,:self.compressedWidth*self.compressedWidth*int(self.compressedChannel/self.memsize)*self.MemtopK], (sz_w, sz_h)).long()
                #print(indices.shape)
    ...
    def update_HERDING_Storage_epoch(self, train_loader):
        ...
        for i, (inputs, target) in enumerate(train_loader):
            ...
            read = read.detach().cpu().reshape(inputs.size(0),self.compressedWidth*self.compressedWidth*self.compressedChannel)
        ...
        for ucls in uni_class_list:
            ...
            print('storing class id: ' + str(int(ucls)) + ' for num of samples = ' + str(self.StorageAttRead[ucls].size(0)))
        ...
        for i, (inputs, target) in enumerate(dataloader):
            ...
            for bat in range(att_read.size(0)):
                ...
                if len(self.viz_read_att[label]) < self.viz_NumEgs:
                    ...
                    #print(att_read[bat,:].shape)
                    #print(int(self.compressedChannel/self.memsize))
                    #print(self.compressedWidth)
                    att_read_bat = att_read[bat,:].view(1,self.compressedWidth,self.compressedWidth,int(self.compressedChannel/self.memsize),self.MemNumSlots)
                    #find top retireved memory address
                    att_read_ind = torch.argsort(att_read_bat, dim=4, descending=True)
                    att_read_ind = att_read_ind[:,:,:,:,0:self.MemtopK]
                    att_read_bat = att_read_ind.view(1,self.compressedWidth,self.compressedWidth,int(self.compressedChannel/self.memsize),self.MemtopK)
