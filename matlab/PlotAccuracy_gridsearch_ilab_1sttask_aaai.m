...
%resultrootdir = '/home/mengmi/Desktop/ilab2mlight_outputs/';
resultrootdir = '/home/mengmi/Desktop/aaai/aaai_results/firstResults_March23/ilab2mlight_outputs/';
...
modellist = {'AugMem','EWC','GEM','iCARL','L2','MAS','NaiveRehearsal','SI','Stable','GSS','BIC','COPE','lwf','NormalNN','NormalNNupper'};
%gridsearchlist = {'EWC','GEM','L2','MAS','NaiveRehearsal','SI'};
legendname={'HAMN (ours)','EWC','GEM','iCARL','L2','MAS','NaiveRehearsal','SI','Stable','GSS','BIC','COPE','lwf','lowerBound','upperBound'};
...
for type =  [2 3] %[1:length(ExpTypeList)]
    ...
    for model = modellist(selectedModel)
        ...
        if strcmp(model, 'NormalNNupper')
            testresult = readtable([resultrootdir typecase '/NormalNN_ResNet18_offline/' 'test_task1.csv']);
...
%                 testresult = readtable([resultrootdir typecase '/' model '_gridsearch/' testresult '/test.csv']);
            ...
            testresult = readtable(['/home/mengmi/Desktop/aaai/Rushikesh_results_1stAccu/' model '/1stTask_' model '_ilab_' temptypecase '.csv']);    
...
%             else
                ...
                testresult = readtable([resultrootdir typecase '/' model '_ResNet18/' 'test_task1.csv']);
%             end
                testresult = testresult{:,:};
        end
        ...
        if strcmp(model,'Stable') || strcmp(model,'GSS') || strcmp(model,'COPE')  || strcmp(model,'lwf') || strcmp(model,'BIC') 
            selectedrows = [1:10];
        else
    ...
    plot([1:7], 0.5./[1:7]*100,'k--','LineWidth',2);
    cond = [cond; temp];
    ...
    %else
        legend([legendname(selectedModel) 'Chance'], 'FontSize',16,'FontWeight','bold','Location','southeastoutside');
    ...
    print(hb,['Figure/performance_' typecase '_ilab_1sttask' printpostfix],printmode,printoption);
...
%plot([0 ngroups+0.5],[nanmean(0.5./[1:7]*100) nanmean(0.5./[1:7]*100)],'k--','LineWidth',2);
...
LegName = {'Class-iid','Class-instance'}; 
legend(LegName,'Location','Northwest','FontSize', 12);
...
%get(hb,'Position')
set(hb,'Position',[675   549   570   413]);
% printpostfix = '.eps';
...
pos = get(hb,'Position');
set(hb,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)]);
print(hb,['Figure/fig_ilab_1stTask_accu' printpostfix],printmode,printoption);
