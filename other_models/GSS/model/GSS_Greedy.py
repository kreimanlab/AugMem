...
class Net(nn.Module):
    def __init__(self,
        ...
        self.is_cifar = ('toybox' in args.data_file)
        if self.is_cifar:
    ...
    def forward(self, x, t=0):
        ...
        output = self.net(x)
        #print("output shape in GSS_greedy")
        #print(output.size())
        return output
        ...
        if self.mem_cnt == self.n_memories :
            ...
            if self.sampled_memory_data is not None and self.n_sampled_memories<=self.sampled_memory_data.size(0):#buffer is full
                ...
                if (batch_sim)<self.sim_th:
                    ...
                    print("scaled_batch_item_sim and buffer_repl_batch_sim size")
                    print(scaled_batch_item_sim.size(),buffer_repl_batch_sim.size())
